...
myCurrentPos = None
myPickInfo = None
zeroDividePrevent = None

# 4개의 점으로 구성되는 곡선의 구간이 차지하는 시간은 0.5초 (1cycle = 3 sec)
def pOnSplineAtTime(control_points, time):
    index = ((int)((time % 3) / 0.5))
    index -= 1
    fourPoints = []
    for i in range(4):
        listIndex = (index + i) % len(control_points)
        fourPoints.append(control_points[listIndex])
    M1 = np.array(
        [
            [2., -2., 1., 1.],
            [-3., 3., -2., -1.],
    ...
    M3 = np.array(fourPoints,float)
    Coef = M1 @ M2 @ M3
    timeFlow = (time % 0.5) * 2
    T = np.array([timeFlow**3, timeFlow**2, timeFlow, 1])
    M = T @ Coef
    return M
...
def grdOnSplineAtTime(control_points, time):
    ...
    M3 = np.array(fourPoints)
    Coef = M1 @ M2 @ M3
    timeFlow = (time % 0.5) * 2
    T = np.array([3 * timeFlow**2, 2 * timeFlow, 1, 0])
    M = T @ Coef
    return M

# one arccos result of two angles makes unexpected error 

'''def myRotate(gradient):
    v1 = np.array([gradient[0],0.,gradient[2]])
    v1_size = np.sqrt(v1 @ v1)
    v2_size = np.sqrt(gradient @ gradient)
    cos1 = (np.array([0., 0., 1.]) @ v1) / v1_size
    cos2 = (gradient @ v1) / (v1_size * v2_size)
    angle1 = np.arccos(cos1)
    angle2 = -np.arccos(cos2)
    T1 = np.array(
        [
            [np.cos(angle1), 0., np.sin(angle1), 0.],
            [0., 1., 0., 0.],
            [-np.sin(angle1),0., np.cos(angle1), 0.],
            [0., 0., 0., 1.]
        ])
    T2 = np.array(
        [
...
def myRotate(gradient):
    v1 = gradient
    v1 = v1 / np.sqrt(v1 @ v1)
    v2 = np.array([gradient[0],0.,gradient[2]])
    v2 = v2 / np.sqrt(v2 @ v2)
    x_axis = np.cross(v1,v2)
    if(x_axis @ x_axis == 0) :
        return zeroDividePrevent
    else:
        x_axis = x_axis / np.sqrt(x_axis @ x_axis)
        if(gradient[1] < 0):
            x_axis = -x_axis
        y_axis = np.cross(v1,x_axis)
        z_axis = v1
        M = np.array(
            [
                np.append(x_axis,0.),
...
def display():
    global cameraIndex, cow2wld, control_points, control_points_matrix, cow2wld_init, animStartTime, pickInfo, zeroDividePrevent
    ...
    if (len(control_points) == 6):
        # 1 cycle = 3 sec 기준으로,,
        if (animStartTime != None): 
            animTime = glfw.get_time() - animStartTime
            # M = makeSpline(control_points)
            if(animTime < 9):
                T = pOnSplineAtTime(control_points, animTime)
                gradient = grdOnSplineAtTime(control_points, animTime)
                R = myRotate(gradient)
                zeroDividePrevent = R
                cow2wld = control_points_matrix[0]
                # translation part -> T : point on spline
                cow2wld = R @ cow2wld
                setTranslation(cow2wld,T)
                #print(R) 
                animTime = glfw.get_time() - animStartTime
                ...
                control_points = []
                control_points_matrix = []
                cow2wld = cow2wld_init
                #setTranslation(cow2wld, pickInfo.cowPickPositionLocal)
    else:
        drawCow(cow2wld, cursorOnCowBoundingBox);
        for i in control_points_matrix:
            drawCow(i, False);														# Draw cow.
...
def initialize(window):
    global cursorOnCowBoundingBox, floorTexID, cameraIndex, camModel, cow2wld, cowModel, cow2wld_init
    cursorOnCowBoundingBox=False;
    # Set up OpenGL state
...
def onMouseButton(window,button, state, mods):
    global isDrag, V_DRAG, H_DRAG, control_points, control_points_matrix, myCurrentPos, animStartTime, pickInfo
    GLFW_DOWN=1;
    GLFW_UP=0;
    ...
    if button == glfw.MOUSE_BUTTON_LEFT:
        if state == GLFW_DOWN:
            if isDrag==H_DRAG:
                ...
                if len(control_points) < 6 :
                    pickInfo=myPickInfo
                    control_points_matrix.append(cow2wld)
                    control_points.append(myCurrentPos)
                    if len(control_points) == 6:
...
def onMouseDrag(window, x, y):
    global isDrag,cursorOnCowBoundingBox, pickInfo, cow2wld, myCurrentPos, myPickInfo
    if isDrag: 
        ...
        if  isDrag==V_DRAG:
            ...
            if cursorOnCowBoundingBox:
                ...
                M=np.eye(4)
                setTranslation(M, [0,currentPos[1]-pp.cowPickPosition[1],0])
                cow2wld=M@pp.cowPickConfiguration;


                nextcowPickPosition=[pp.cowPickPosition[0],currentPos[1],pp.cowPickPosition[2]]
                nextcowPickLocalPos=transform(np.linalg.inv(cow2wld),nextcowPickPosition)
                pickInfo=PickInfo( pp.cursorRayT, nextcowPickPosition, cow2wld, nextcowPickLocalPos)
            print('vdrag')
            ...
            if cursorOnCowBoundingBox:
                ...
                myPickInfo = PickInfo(True, currentPos, cow2wld, cowPickLocalPos)
    else:
        ray=screenCoordToRay(window, x, y)
